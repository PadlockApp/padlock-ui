...
export type Ffs = {
    ...
    newAddr: (name: string, type?: "bls" | "secp256k1" | undefined, makeDefault?: boolean | undefined) => Promise<ffsTypes.NewAddrResponse.AsObject>;
    getDefaultCidConfig: (cid: string) => Promise<ffsTypes.GetDefaultCidConfigResponse.AsObject>;
    getCidConfig: (cid: string) => Promise<ffsTypes.GetCidConfigResponse.AsObject>;
    ...
    watchLogs: (handler: (event: ffsTypes.LogEntry.AsObject) => void, cid: string, ...opts: ffsOptions.WatchLogsOption[]) => () => void;
    replace: (cid1: string, cid2: string) => Promise<ffsTypes.ReplaceResponse.AsObject>;
    pushConfig: (cid: string, ...opts: ffsOptions.PushConfigOption[]) => Promise<ffsTypes.PushConfigResponse.AsObject>;
    ...
    createPayChannel: (from: string, to: string, amt: number) => Promise<ffsTypes.CreatePayChannelResponse.AsObject>;
